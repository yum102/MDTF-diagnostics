...
* ``INPUT_FILE``: Path to a user configuration file that sets options listed here. This can be a JSON file of the form given in `sample_input.jsonc <https://github.com/NOAA-GFDL/MDTF-diagnostics/blob/main/sample_input.jsonc>`__ (which is intended to be copied and used as a template), or a text file containing command-line arguments. Options set explicitly on the command line will still override settings in this file.
