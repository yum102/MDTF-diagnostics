...
class TestVariableTranslator(unittest.TestCase):
    ...
    @mock.patch('framework.configs.util.read_json', return_value = {
...
class TestPathManagerPodCase(unittest.TestCase):
    ...
    @mock.patch('framework.data_manager.util.check_required_dirs')
    def test_pathmgr_model(self, mock_check_required_dirs):
        config = configs.ConfigManager()
    ...
    @mock.patch('framework.data_manager.util.check_required_dirs')
    def test_pathmgr_pod(self, mock_check_required_dirs):
        config = configs.ConfigManager()
        case = DataManager(self.case_dict)
        pod = Diagnostic('AA')
