...
def checksum_in_subtree_1(rootdir, reference_subdirs, exclude_exts=[]):
    ...
    for d1 in reference_subdirs:
        ...
        checksum_dict[d1] = checksum_files_in_subtree(p1, exclude_exts)
    return checksum_dict
...
def make_output_data_dict(rootdir, case_list, exclude_exts=[]):
    ...
    for c in case_list:
        ...
        for d2 in c['pod_list']:
            ...
            checksum_dict[d1][d2] = checksum_files_in_subtree(p2, exclude_exts)
    return checksum_dict
...
def get_test_data_configuration():
    cwd = os.path.dirname(os.path.realpath(__file__)) # gets dir of currently executing script
    case_list = util.read_json(os.path.join(cwd,'pod_test_configs.json'))
    models = []
    pods = []
